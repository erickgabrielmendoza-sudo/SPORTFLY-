    ...
    <div className="min-h-screen bg-slate-50 selection:bg-blue-100">
      ...
      <main>
        ...
        <section className="relative bg-[#0a192f] py-28 px-4 text-center overflow-hidden">
          ...
          <div className="relative z-10 max-w-4xl mx-auto">
            ...
            <h1 className="text-6xl md:text-8xl font-sport mb-10 text-white tracking-tighter drop-shadow-2xl flex items-center justify-center">
              <span className="relative inline-block">
                S
                <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-amber-400 opacity-80">
                  <Trophy size={32} strokeWidth={3} />
                </span>
              </span>
              PORT
              <span className="text-amber-400 relative inline-block">
                <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-white rotate-45">
            ...
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto relative group">
              <div className="flex justify-center gap-4 mb-4">
                <button 
                  type="button"
                  onClick={() => setSearch(p => ({ ...p, flightMode: 'roundTrip' }))}
                  className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${search.flightMode === 'roundTrip' ? 'bg-amber-400 text-blue-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                >
                  Vuelo Redondo
                </button>
                <button 
                  type="button"
                  onClick={() => setSearch(p => ({ ...p, flightMode: 'oneWay' }))}
                  className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${search.flightMode === 'oneWay' ? 'bg-amber-400 text-blue-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                >
                  Solo Ida
                </button>
              </div>
      ...
      {showProfile && (
        <ProfileView 
          ...
          onLogout={() => setUser(p => ({...p, isAuthenticated: false, isPremium: false}))} 
          onViewWallet={setWallet}
          onViewPayout={() => setShowPayout(true)}
        />
      )}
      {selectedPackage && (
        <BookingSummary 
          ...
          onConfirmPurchase={(p) => setUser(u => ({
            ...u, 
            travelHistory: [p, ...u.travelHistory], 
            purchaseCount: u.purchaseCount + 1,
            payoutConfig: u.payoutConfig ? {
              ...u.payoutConfig,
              pendingPayout: u.payoutConfig.pendingPayout + 560,
              totalEarnings: u.payoutConfig.totalEarnings + 560,
            } : u.payoutConfig
          }))} 
        />
      )}
      ...
      {showPayout && (
        <PayoutModal 
          ...
          onLinkCard={(cardHolder, cardLast4) => {
            setUser(prev => ({
              ...prev,
              payoutConfig: {
                ...prev.payoutConfig!,
