...
import BrandToolkit from './components/BrandToolkit.tsx';
import { SearchState, TravelPackage, UserProfile } from './types.ts';
import { searchSportPackages, generateItinerary } from './geminiService.ts';
import { SAMPLE_PACKAGES } from './constants.ts';
...
const App: React.FC = () => {
  ...
  const [itinerary, setItinerary] = useState<{title: string, content: string} | null>(null);
  ...
  const handleViewItinerary = async (pkg: TravelPackage) => {
    try {
      const content = await generateItinerary(pkg.event.name, pkg.event.city);
      setItinerary({ title: `Itinerario: ${pkg.event.name}`, content });
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      {selectedPackage && (
        <BookingSummary 
          ...
          onConfirmPurchase={(pkg, saved) => setUser(u => ({...u, travelHistory: [pkg, ...u.travelHistory], purchaseCount: u.purchaseCount + 1, totalSaved: u.totalSaved + saved}))} 
        />
      )}
      {wallet && <WalletModal pkg={wallet} onClose={() => setWallet(null)} />}
      {itinerary && <ItineraryModal title={itinerary.title} content={itinerary.content} onClose={() => setItinerary(null)} />}
